...
/* ─────────────────────────────────────────
   CHALLENGE CARDS — Modal Pop-up
───────────────────────────────────────── */
const challengeModal = $('#challengeModal');
const challengeModalTitle = $('#challengeModalTitle');
const challengeModalNumber = $('#challengeModalNumber');
const challengeModalIcon = $('#challengeModalIcon');
const challengeModalPreview = $('#challengeModalPreview');
const challengeModalContent = $('#challengeModalContent');
const challengeModalClose = $('#challengeModalClose');

function openChallengeModal(card) {
  const title = $('.c-title', card)?.textContent?.trim() || 'Challenge';
  const number = $('.c-number', card)?.textContent?.trim() || '';
  const icon = $('.c-icon', card)?.textContent?.trim() || '💡';
  const preview = $('.c-preview', card)?.textContent?.trim() || '';
  const body = $('.c-body', card)?.innerHTML || '';

  challengeModalTitle.textContent = title;
  challengeModalNumber.textContent = number;
  challengeModalIcon.textContent = icon;
  challengeModalPreview.textContent = preview;
  challengeModalContent.innerHTML = body;
...
document.addEventListener('click', (event) => {
  const btn = event.target.closest('.c-toggle');
  if (!btn) return;
  ...
  if (card) openChallengeModal(card);
});

challengeModalClose?.addEventListener('click', closeChallengeModal);
$$('[data-close-modal]').forEach(el => el.addEventListener('click', closeChallengeModal));

document.addEventListener('keydown', (event) => {
  if (event.key === 'Escape' && challengeModal?.classList.contains('open')) {
    closeChallengeModal();
  }
});
...
const QUESTIONS = [
  ...
  {
    q: 'Why is a good credit history important?',
    opts: ['It helps lenders judge whether you repay on time', 'It increases your salary automatically', 'It replaces your passport', 'It shows how much cash you carry'],
    ans: 0,
    exp: 'A good credit history helps banks and lenders judge how reliably you repay borrowed money. It can affect loans, installment plans, and mortgages.'
  },
  {
  ...
  },
  {
    ...
    opts: ['1 month of expenses', '3–6 months of living expenses', 'Exactly 4,750,000 ₸', '12 months of expenses'],
    ans: 1,
  ...
  {
    q: 'Which habit best supports a healthy credit history?',
    opts: ['Paying installments and card bills on time', 'Applying for many loans at once', 'Ignoring repayment dates', 'Borrowing the maximum every month'],
    ans: 0,
    exp: 'Paying on time is one of the most important habits for keeping a healthy credit history and avoiding unnecessary penalties.'
  },
  {
  ...
  },
  {
    ...
    opts: [
      ...
      'Spend no more than 72,000 ₸ a day'
    ],
    ans: 2,
